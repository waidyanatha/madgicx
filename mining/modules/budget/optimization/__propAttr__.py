...
#             self.dataDir = pkgConf.get("CWDS","DATA")
    ...
    def prediction(self) -> DataFrame:
        """
        Description:
            prediction @property and @setter functions. make sure it is a valid realm
        Attributes:
            prediction in @setter will instantiate self._realm  
        Returns :
        ...
        try:
            if self._prediction is None or not isinstance(self._prediction, list)\
                or len(self._prediction)<=0:
                raise ValueError("No predictions to share")
        except Exception as err:
            logger.error("%s %s \n",__s_fn_id__, err)
            logger.debug(traceback.format_exc())
            print("[Error]"+__s_fn_id__, err)
    ...
    def prediction(self,prediction) -> DataFrame:
        ...
        try:
            if prediction is None or not isinstance(prediction, list) or len(prediction)<=0:
                raise ValueError("Invalid %s prediction" % (type(prediction)))
